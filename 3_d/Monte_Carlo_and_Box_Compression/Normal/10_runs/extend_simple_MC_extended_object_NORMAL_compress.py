...
data=pylab.loadtxt('traj_MC_NORMAL_1.75_1.dat')
x=data[:,0]
y=data[:,1]
z=data[:,2]
alpha=data[:,3]
...
ax.scatter(extended_x,extended_y,extended_z, color='red', alpha=0.1)
#plt.show()
'''
extended_x=[]
extended_y=[]
    ...
    for i in range(1,MC):
        ...
        for j in range(0,N):
            ...
            #o_lap=overlap(xnew,ynew,thetanew,x[j],y[j],theta[j])
                o_lap=overlap(xnew,ynew,znew,alphanew,betanew,gammanew,x[j],y[j],z[j],alpha[j],beta[j],gamma[j],L)
        ...
        if check==0:
            ...
            acc=acc+1
        if i%1000==0:
            s='traj_MC_NORMAL_'+str(round(L,3))+'_'+str(int(i/1000))+'.dat'
            f=open(s,'w')
            for k in range(0,N):
