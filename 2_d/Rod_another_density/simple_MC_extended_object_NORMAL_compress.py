       ...
       #     xnew=xnew-L
        ...
        for j in range(0,N):
            ...
            if number!=j:
                ...
                #o_lap=overlap_ML(xnew,ynew,thetanew,x[j],y[j],theta[j],model,L)
                if o_lap==1:
                    check=1
        if check==0:
            x[number]=xnew
            ...
            acc=acc+1
        if i%freq==0:
            s='traj_MC_'+str(filename)+'_'+str(round(L,2))+'_'+str(int(i/freq))+'.dat'
            f=open(s,'w')
            for k in range(0,N):
...
for i in range(0,60):
    ...
    mcr=1.21
    for j in range(0,30):
        x,y,theta, L,acc=monte_carlo_move(200,mct,mct,mcr,L,x,y,theta,1000, 'NORMAL')
        if acc>0.5:
            break
        else:
            if mct>0.2:
    ...
    x,y,theta,L,acc=monte_carlo_move(100000,mct,mct,mcr,L,x,y,theta,1000,'NORMAL')
