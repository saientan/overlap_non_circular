...
def RUN_ALL(model,model_name, Area_frac):
    ...
    def monte_carlo_move(MC,dx,dy,dtheta,L,x,y,thetai, ML_Model,model_name):
        ...
        for i in range(1,MC):
            ...
            if i%1000 == 0:
                s = './'+ 'ML_' + model_name + '_' + str(round(L,2)) + '/traj_MC_ML_'+ str(model_name) + '_' + str(round(L,2)) + '_' + str(int(i/1000)) + '.dat'
                f = open(s,'w')
                for k in range(0,N):
    ...
    abcd = open('./' + 'ML_' + model_name + '_' + str(round(L,2)) + '/time_taken_' + str(model_name), 'w')
